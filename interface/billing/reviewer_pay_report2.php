<?php
//Administration level CBHA reviewer pay report
...
<title><?xl('CBHA Reviewer Pay Report','e')?></title>
</head>

<body class="body_top">

<span class='title'><?php xl('Report','e'); ?> - <?php xl('CBHA Review Pay Report','e'); ?></span>

<form method='post' action='reviewer_pay_report2.php' id='theform'>
  ...
  if ($_POST['form_refresh']) {
    ...
    if ($INTEGRATED_AR) {
      ...
      $irow = 0;
        ...
        $query = "SELECT b.fee, b.pid, b.encounter, b.code_type, b.code, b.review_units, b.modifier, " .
          "fe.date, fe.id AS trans_id, fe.reviewer_id AS docid, fe.invoice_refno " .
          "FROM billing AS b " .
        ...
        if ($form_doctor) {
          $query .= " AND fe.reviewer_id = '$form_doctor'";
        }
        /************************************************************/
        //
        $res = sqlStatement($query);
...
$amount2 = 0;
        $amount2 -= $row['amount'];
        $amount1 = $row['review_units']*8.125;
