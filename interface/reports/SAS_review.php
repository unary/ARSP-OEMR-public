...
<title><?xl('SAS Reviewer Pay Report','e')?></title>
</head>

<body class="body_top">

<span class='title'><?php xl('Report','e'); ?> - <?php xl('SAS Review Pay Report','e'); ?></span>

<form method='post' action='SAS_review.php' id='theform'>
  ...
  <td width='660px'>
	...
	<table class='text'>
		<tr>
			...
			<td>
				<?php
					...
					//echo "<input type='hidden' name='form_doctor' value='8'>";
		...
		<tr>
			...
			<td>
			   <input type='text' name='form_cptcode' size='5' value='0000%'
				title='<?php xl('SAS code','e'); ?>'
				<?php if ($GLOBALS['simplified_demographics']) echo "style='display:none'"; ?>>
			</td>
			<td>
			   <input type='text' name='form_modifier' size='2' value=''
				title='<?php xl('No Modifiers Allowed','e'); ?>'
				<?php if ($GLOBALS['simplified_demographics']) echo "style='display:none'"; ?>>
			</td>
  ...
  if ($_POST['form_refresh']) {
    ...
    if ($INTEGRATED_AR) {
      ...
      $query = "SELECT a.pid, a.encounter, a.post_time, a.code, a.modifier, a.pay_amount, " .
        "fe.date, fe.id AS trans_id, fe.reviewer_id AS docid, fe.invoice_refno, s.deposit_date, s.payer_id, " .
        "b.reviewer_id, b.review_units " .
        ...
        "WHERE a.pay_amount != 0 AND a.account_code!='PCP' AND b.review_units != 0 AND ( " .
      ...
      if ($form_doctor) {
        $query .= " AND ( b.reviewer_id = '$form_doctor' " .
          " AND " .
          "fe.reviewer_id = '$form_doctor' ) ";
      }
      /**************************************************************/
      //
      $res = sqlStatement($query);
      while ($row = sqlFetchArray($res)) {
        ...
        $arows[$key]['review_units'] = $row['review_units'];
      ...
      for ($irow = 0; $irow < SLRowCount($t_res); ++$irow) {
        ...
        if ($row['trans_id'] == $last_trans_id) {
          ...
          $tmp = sqlQuery("SELECT reviewer_id FROM form_encounter WHERE " .
            "pid = '$patient_id' AND encounter = '$encounter_id' " .
            "ORDER BY id DESC LIMIT 1");
          $main_docid = $tmp['reviewer_id'] + 0;
    ...
    foreach ($arows as $row) {
      ...
      if ($row['project_id']) {
        ...
        $amount1 = $row['review_units']*12.5;
      $units = $row['review_units'];       
