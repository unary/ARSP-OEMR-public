...
//Modified for BH use, dropped references to SQL Ledger -Art Eaton OEMR.org
...
if ($_POST['form_refresh'] || $_POST['form_export'] || $_POST['form_csvexport']) {
  if ($is_ins_summary) {
    ...
    $form_cb_thecounselor    = false;
    $form_cb_billdate    = false;
    $form_cb_city     = false;
    $form_cb_ins1     = false;
    ...
    $form_cb_thecounselor    = $_POST['form_cb_thecounselor']    ? true : false;
    $form_cb_billdate    = $_POST['form_cb_billdate']    ? true : false;
  ...
  $form_cb_adate    = false;
  $form_cb_policy   = false;
  $form_cb_thecounselor    = true;
  $form_cb_billdate    = true;
  $form_cb_city     = false;
  $form_cb_ins1     = true;
  $form_cb_referrer = false;
  $form_cb_idays    = false;
...
if ($form_cb_thecounselor   ) ++$initial_colspan;
if ($form_cb_billdate   ) ++$initial_colspan;
...
function endPatient($ptrow) {
  ...
  if ($_POST['form_export']) {
    ...
    echo " ";                      // filler
    echo sprintf("%-15s", substr($ptrow['thecounselor'], 0, 15));
    echo substr($ptrow['dos'], 5, 2) . substr($ptrow['dos'], 8, 2) . substr($ptrow['dos'], 2, 2);
   echo substr($ptrow['billdate'], 5, 2) . substr($ptrow['billdate'], 8, 2) . substr($ptrow['billdate'], 2, 2); 
			...
			<td class='label'>
				<table>
					...
					<tr>
						...
						<td>
						   <input type='checkbox' name='form_cb_pid'<?php if ($form_cb_pid) echo ' checked'; ?>>
						   <?php xl('ID','e') ?>&nbsp;
						</td>
						<td>
						...
						<td>
						   <input type='checkbox' name='form_cb_thecounselor'<?php if ($form_cb_thecounselor) echo ' checked'; ?>>
						   <?php xl('Assigned Counselor','e') ?>&nbsp;
						</td>
						<td>
...
if ($_POST['form_refresh'] || $_POST['form_export'] || $_POST['form_csvexport']) {
  ...
  $where = "";
    ...
    while ($erow = sqlFetchArray($eres)) {
      ...
      $billdate = substr($erow['billdate'], 0, 10);
      ...
      $row['thecounselor']     = $erow['thecounselor'];
...
<?php } ?>
<?php if ($form_cb_thecounselor) { ?>
  <th>&nbsp;<?php xl('Assigned Counselor','e')?></th>
<?php } ?>
<?php if ($form_cb_city) { ?>
  <th>&nbsp;<?php xl('City','e')?></th>
<?php } ?>
...
<?php
      if ($ptrow['count'] == 1) {
        ...
        if ($form_cb_pid) {
          echo "  <td class='detail'>&nbsp;" . $row['pid'] . "</td>\n";
        }
        if ($form_cb_policy) {
          echo "  <td class='detail'>&nbsp;" . $row['policy'] . "</td>\n";
        }
        if ($form_cb_thecounselor) {
          echo "  <td class='detail'>&nbsp;" . $row['thecounselor'] . "</td>\n";
        }
        if ($form_cb_city) {
...
?>
  ...
  <td class="detail">
   ...
   <td class="detail">
   &nbsp;<?php echo oeFormatShortDate($row['billdate']); ?>
  </td>
<?php if ($form_cb_adate) { ?>
  <td class='detail'>
