...
class TCMAssessment {
	...
	public function checkNewAssessment() {
//		if(!is_array($this->formData) || count($this->formData) === 0)
//			throw new \UnexpectedValueException('Form Data is missing.');
//
//		if(!array_key_exists('assessment-type', $this->formData)
//			|| !in_array($this->formData['assessment-type'], array('initial','update'))
//		)
//			$this->formErrors[] = 'You must select an assessment type (ie: Initial, Annual Update).';
//
//		return count($this->formErrors) === 0;
	...
	public function sqlSaveEncounter() {
		if(property_exists($this->formData, 'Encounter')
			&& is_array($this->formData->Encounter)
		) {
			$this->sqlStatement(
				$this->sqlStatments['UPDATE']['FormEncounter'], 
				array(
					'EncounterId' => $this->formData->EncounterId,
					'ReportDate' => $this->formData->Encounter->ReportDate,
